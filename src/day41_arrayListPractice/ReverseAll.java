...
Output: {"det", "klat", "nrael"}
    ...
    public static ArrayList<String> reverseAll(ArrayList<String>list){
        ...
        for (String each:list) {
            reverseWords.add(StringUtil.reverse(each));// calling the reverse method from the stringutil class to reverse each word, each element, then we are adding the reversed word into the 'reverseWords' ArrayList
